...
- `assets/paul-passport.png` — Profile image used in the hero card. Add `assets/paul-passport-260.webp`/`assets/paul-passport-520.webp` for better performance and `-260`/`-520` sizes for responsive `srcset`.
...
- - Assets: Replace `assets/paul-passport.png` in `assets/` and keep the filename to avoid index changes; optionally provide `assets/paul-passport.webp` for improved compression.
